...
will automatically **upgrade** an older version of Camtrap DP to the
latest version. It will also make the data easier to use, by assigning
taxonomic information (found in the metadata) to the observations and
`eventID`s (found in the observations) to the media.
...
[`read_camtrapdp()`](https://inbo.github.io/camtrapdp/reference/read_camtrapdp.md)
and then converted to Darwin Core with
