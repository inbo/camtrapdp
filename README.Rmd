...
Camtrapdp is the R interface to [Camera Trap Data Package (Camtrap DP)](https://camtrap-dp.tdwg.org), a data exchange format for camera trap data. It is designed to read, filter and convert data (including to Darwin Core) before further analysis in e.g. [camtraptor](https://inbo.github.io/camtraptor/) or [camtrapR](https://cran.r-project.org/package=camtrapR).
...
One can also **filter** data, which will automatically filter the related data. For example, here are all the event-based observations that have a media that was marked as favourite:
