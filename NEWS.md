...
* New function `taxonomy()` returns the unique scientific name and associated taxonomic information from the observations (#17). This information is derived from the metadata and added the observations with column prefix `taxon.` in `read_camtrap_dp()` (#37).
